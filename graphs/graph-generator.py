...
class JSONParser:
    ...
    def print_running_time(self):
        print("User-based" if self.is_user_based else "Item-based")
        total_average = 0
        for b in self.data:
            average = sum(d["time"] for d in self.data[b]) / len(self.data[b])
            total_average += average
            print(f" - {b}: {average}")
        
        print("=> Overall average:", total_average/5) 
...
def plot_data(window_title, filename, is_user_based, data_names_2d, data_names_3d=[]):
    ...
    # json_reader.print_running_time()
